...
func add_file_hiden():
	...
	OS.execute("CMD.exe", ["/C", "echo 但我真的想一直一直一起 >> %USERPROFILE%/desktop/file_hiden.txt"],output,true)
	OS.execute("CMD.exe", ["/C", "echo 一起联机，一起看东西，一起听音乐 >> %USERPROFILE%/desktop/file_hiden.txt"],output,true)
	OS.execute("CMD.exe", ["/C", "echo suki！ >> %USERPROFILE%/desktop/file_hiden.txt"],output,true)
